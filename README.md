# Event Ticket Manager (Directus admin)

Se utiliza el gestor de datos [Directus](https://directus.io) para contar rápidamente con una UI de backoffice que nos permite visualizar y actualizar toda la información del modelo de datos relacional.

## Configuración

Luego de clonar este repositorio se deberá crear un archivo `.env` en el mismo directorio para proveer los datos de conexión con la base de datos.

A priori, las entradas de configuración mínima que nos interesa es la siguiente:

```bash
DB_CLIENT="mysql" #usamos mysql
DB_HOST="127.0.0.1"
DB_PORT="3306"
...
Aquí un ejemplo completo de la configuraciónd de entorno que se puede usar en el `.env`:
...
Una vez configurado el `.env` solo debemos ejecutar el comando de inicio en el mismo directorio:

```bash
npx directus start
```

La primera vez nos, aún no instaladas las dependencias, nos pedirá confirmación para hacerlo y luego ya inicia normalmente el proceso.

Listos para navegar la aplicación en http://localhost:8056.
